...
DBM.AddOns.TwinEmps = {
	...
	["OnCombatStart"] = function(delay)
		...
		DBM.StartStatusBarTimer(900, "Enrage", "Interface\\Icons\\Spell_Shadow_UnholyFrenzy");
		
		DBM.StartStatusBarTimer(35, "Teleport", "Interface\\Icons\\Spell_Arcane_Blink");
	...
	["OnEvent"] = function(event, arg1)	
		if ( event == "CHAT_MSG_SPELL_PERIODIC_CREATURE_DAMAGE" and string.find(arg1, DBM_TWINEMPS_EXPLODE_EXPR) ) then
			if DBM.AddOns.TwinEmps.Options.BugExplode then
				DBM.AddSpecialWarning(DBM_TWINEMPS_EXPLODE_ANNOUNCE);
			end
		elseif event == "CHAT_MSG_SPELL_PERIODIC_CREATURE_BUFFS" then -- mob casts a spell
			if (string.find(arg1, DBM_TWINEMPS_CAST_SPELL_1)) then
				...
				DBM.StartStatusBarTimer(35, "Teleport", "Interface\\Icons\\Spell_Arcane_Blink");
				-- DBM.Schedule(25, "DBM.AddOns.TwinEmps.OnEvent", "TeleportWarning", 10);
				DBM.Schedule(30, "DBM.AddOns.TwinEmps.OnEvent", "TeleportWarning", 5);
			end
