...
DBM.AddOns.Battlegrounds = { 
	...
	["OnEvent"] = function(event,  arg1)
		...
		if(event == "CHAT_MSG_SPELL_PERIODIC_SELF_DAMAGE" and string.find(arg1, DBM_BGMOD_LANG.WSG_BOOTS_EXPR) )then
			DBM.StartStatusBarTimer(10, "Speed Boots", "Interface\\Icons\\INV_Boots_02", true);
			...
			for index, value in ipairs(DBM_BGMOD_LANG.AB_TARGETS) do --AB Captures
				if string.find(arg1, value) then
					if string.find(arg1, DBM_BGMOD_LANG["AB_HASASSAULTED"]) or string.find(arg1, DBM_BGMOD_LANG["AB_CLAIMSTHE"]) then
						...
						DBM.EndStatusBarTimer("部落: "..DBM.Capitalize(DBM_BGMOD_EN_TARGET_AB[index]));
						DBM.StartColoredStatusBarTimer(60,  "联盟: "..DBM.Capitalize(DBM_BGMOD_EN_TARGET_AB[index]), "Interface\\Icons\\INV_BannerPVP_02.blp", 0, 0, 1, nil, nil, nil, "Battlegrounds");
					...
					elseif string.find(arg1, DBM_BGMOD_LANG["AB_HASDEFENDEDTHE"]) or string.find(arg1, DBM_BGMOD_LANG["AB_HASTAKENTHE"]) then
						...
						DBM.EndStatusBarTimer("联盟: "..DBM.Capitalize(DBM_BGMOD_EN_TARGET_AB[index]), true);
						DBM.EndStatusBarTimer("部落: "..DBM.Capitalize(DBM_BGMOD_EN_TARGET_AB[index]), true);
					end
				end
			end
		...
		elseif (event == "CHAT_MSG_BG_SYSTEM_HORDE") then
			for index, value in ipairs(DBM_BGMOD_LANG.AB_TARGETS) do --AB Captures
				if string.find(arg1, value) then
					if string.find(arg1, DBM_BGMOD_LANG["AB_HASASSAULTED"]) or string.find(arg1, DBM_BGMOD_LANG["AB_CLAIMSTHE"]) then
						--Initiate a timer w/ value as text string for 60 seconds
						DBM.EndStatusBarTimer("联盟: "..DBM.Capitalize(DBM_BGMOD_EN_TARGET_AB[index]));
						DBM.StartColoredStatusBarTimer(60, "部落: "..DBM.Capitalize(DBM_BGMOD_EN_TARGET_AB[index]), "Interface\\Icons\\INV_BannerPVP_01.blp", 1,0,0, nil, nil, nil, "Battlegrounds");
					elseif string.find(arg1, DBM_BGMOD_LANG["AB_HASDEFENDEDTHE"]) or string.find(arg1, DBM_BGMOD_LANG["AB_HASTAKENTHE"]) then
						--clear a timer for value
						if string.find(arg1, DBM_BGMOD_LANG["AB_HASTAKENTHE"]) then
							if DBM_BGMOD_LANG["AB_TARGETS_ANNOUNCE"] and DBM_BGMOD_LANG["AB_TARGETS_ANNOUNCE"][index] then
		...
		elseif (event == "CHAT_MSG_MONSTER_YELL") then
			...
			if string.find(arg1,DBM_BGMOD_LANG["AV_IVUS"]) then 
				DBM.StartStatusBarTimer(600,  "森林之王刷新", "Interface\\Icons\\INV_BannerPVP_02.blp");
			elseif string.find(string.lower(arg1),string.lower(DBM_BGMOD_LANG["AV_ICEY"])) then
				DBM.StartStatusBarTimer(600,  "冰雪之王刷新", "Interface\\Icons\\INV_BannerPVP_01.blp");
			end
		...
		elseif (event == "CHAT_MSG_BG_SYSTEM_NEUTRAL") then
			if (arg1 == DBM_BGMOD_LANG["WSG_START60SEC"]
			 ...
			 or arg1 == DBM_BGMOD_LANG["AV_START60SEC"] ) then
					...
					DBM.StartStatusBarTimer(60.3, "Begins", "Interface\\Icons\\Spell_Magic_PolymorphChicken", true);
			...
			elseif (arg1 == DBM_BGMOD_LANG["WSG_START30SEC"]
			     ...
			     or arg1 == DBM_BGMOD_LANG["AV_START30SEC"] ) then
					...
					if not DBM.GetStatusBarTimerTimeLeft("Begins") then
						DBM.StartStatusBarTimer(60.3, "Begins", "Interface\\Icons\\Spell_Magic_PolymorphChicken");
					end
					DBM.UpdateStatusBarTimer("Begins", 30, 60.3);
			...
			if string.find(arg1, DBM_BGMOD_LANG["WINS"]) then
				...
				for index, value in ipairs(DBM_BGMOD_EN_TARGET_AV) do
					DBM.EndStatusBarTimer("部落: "..DBM_BGMOD_EN_TARGET_AV[index], true);
					DBM.EndStatusBarTimer("联盟: "..DBM_BGMOD_EN_TARGET_AV[index], true);
				end
