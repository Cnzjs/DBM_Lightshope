...
DBM.AddOns.Ossirian = { 
	...
	["OnCombatStart"] = function(delay)
		DBM.StartStatusBarTimer(20, "6_Winds", "Interface\\Icons\\Spell_Nature_Cyclone");
		DBM.StartStatusBarTimer(30, "Ossirians Curse", "Interface\\Icons\\Spell_Shadow_CurseOfTounges");
	end,
	["OnEvent"] = function(event, arg1) 
		...
		if ((time() - DBM.AddOns.Ossirian.LastCurse) > 10
		    ...
		    and string.find(arg1,DBM_OSSIRIAN_CURSE_EXPR) ) then
				...
				DBM.StartStatusBarTimer(18, "Ossirians Curse", "Interface\\Icons\\Spell_Shadow_CurseOfTounges");
		...
		if ( event == "CHAT_MSG_SPELL_PERIODIC_CREATURE_DAMAGE" ) then
			...
			if ( sArg1 ) and ( sArg ~= "Expose" ) then
				...
				if sArg1 == "奥术" then sArg1 = "Arcane"; --you may _never_ use localized strings to start status bars...use the DBM_SBT table to localize status bar strings!
				elseif sArg1 == "火焰" then sArg1 = "Fire";
				elseif sArg1 == "冰霜" then sArg1 = "Frost";
				elseif sArg1 == "自然" then sArg1 = "Nature";
				elseif sArg1 == "暗影" then sArg1 = "Shadow";
				end
		...
		if ( event == "CHAT_MSG_SPELL_PERIODIC_SELF_DAMAGE" or event == "CHAT_MSG_SPELL_PERIODIC_FRIENDLYPLAYER_DAMAGE" or event == "CHAT_MSG_SPELL_PERIODIC_PARTY_DAMAGE" )then
			if string.find (arg1,DBM_OSSIRIAN_WIND_EXPR) then
				DBM.EndStatusBarTimer("6_Winds");
				DBM.StartStatusBarTimer(15, "6_Winds", "Interface\\Icons\\Spell_Nature_Cyclone");
			end
		end
